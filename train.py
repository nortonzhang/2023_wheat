...
data_path = "E:\\04-design\\2023_wheat\\data\\wheatData_split910" # todo 数据集路径
# 提前划分数据集
# 超参数设置
params = {
    # 'model': 'vit_tiny_patch16_224',  # 选择预训练模型
    # 'model': 'resnet101d',  # 选择预训练模型
...
class SELFMODEL(nn.Module):
    def __init__(self, model_name=params['model'], out_features=params['num_classes'],
        ...
        if model_name[:3] == "res":
            ...
            n_features = self.model.Sequential.in_features          # 修改全连接层数目
            self.model.Sequential = nn.Linear(n_features, out_features)  # 修改为本任务对应的类别数目
        else:
...
if __name__ == '__main__':
    ...
    for epoch in range(1, params['epochs'] + 1):  # 开始训练
        ...
        if val_acc >= best_acc:
            ...
            # # 读取 ONNX 模型
            # onnx_model = onnx.load('resnet50d_wheat23.onnx')
            # # 检查模型格式是否正确
